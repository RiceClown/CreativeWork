...
public:
	...
	int GetAdjMatrixElem(int i, int j) { return adjMatrix[i][j]; }//вернет элемент матрицы смежности
	int GetVertListSize() { return vertList.size(); }		//вернЄт кол-во вершин графа
	int GetVertPos(const int& vertex);						//вернЄт позицию вершины
	int GetcountEdges();									//вернЄт кол-во ребер
...
int** reductionLines(int** oldmatrix);	//редукци¤ строк и столбцов
...
void drawCoveredCircle(int x, int y, int R);			//зменит(нарисует поверх) цвет круга, если курсос наведен на него
void drawText(int nom, int x1, int y1);					//нарисует текст
void drawLine(int text, int x0, int y0, int x1, int y1);//нарисует соедин¤ющую линию
void drawVertex(int n);									//нарисует вершину и текст (круг и текст)
void drawPathVertex(int n);								//визуализаци¤ пути коммиво¤жера
...
bool checkCommi(int** matrix);							//проверка на возможность задачи коммиво¤жера
...
bool cross(int x11, int x12, int y11, int y12, int x21, int x22, int y21, int y22);	//нахождение точки пересечени¤ отрез. линии и ребра (ребер)
void deleteEdgeByCut();									//удаление выбранного (или выбранных) ребра (ребер)

void moveMouse(int x, int y);							//отслеживание движени¤ мыши
void clickMouse(int button, int state, int x, int y);	//отслеживание нажатий мыши
void reshape(int w, int h);								//ѕересборка формы
